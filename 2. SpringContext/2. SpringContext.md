...
When the Configuration class has been created **we have to tell Spring Context to use it**, it can happen like this:

```java 
public class Main {
  public static void main(String[] args) {
     var context = new AnnotationConfigApplicationContex(ProjectConfig.class);                
  }
}
```

In order to retrieve instance from the Context we have to get its reference using **context.getBean()**, like this:
```java
public class Main {
  public static void main(String[] args) {
     var context = new AnnotationConfigApplicationContex(ProjectConfig.class);
...
*You are not limited only to object instances you can add Integer or String for example in the Context. This is done the same way with creating a method in the Configuration class that returns either String or Integer or some other type*


### Adding and getting multiple beans of the same instance into the Context 

*For the example again we have class Parrot with only one attribute String name and getters/setters for it*

In order to have more instances added to the Context, more methods annotated with @Beans shall be added to the Configuration class, for example:
```java
@Configuration
public class ProjectConfig {
...
Now when we have multiple instance of a class, getting a specific from the context will be slightly different, take a look at this
```java
public class Main {
  public static void main(String[] args) {
    var context = new 
...
```
**Now getting the bean from the context, requires its name**

**Remember: The bean's name is the same as the method from which it was created**

#### If we want to set a name to the bean, there are multiple ways to do that:
- ```@Bean(name = "genji")```
- ```@Bean(value = "genji")```
- ```@Bean("genji")```
When you have **multiple beans of the same kind in the Spring context** you can make one of them **primary**. To achieve this you have to use the ```@Primary``` annotation
...
*A primary bean is the one Spring will choose if it has multiple options and you dont specify a name*

## Using stereotypes annotations to add beans to the Spring Context 

With stereotypes annotations, you add the annoitation above the class for which you need to have an instance in the Spring context.

When the app creates the Spring context, Spring creates an instance
of the class you marked as a ```@Component``` and adds that instance to its context. 

*Even with this approach we will be using Configuration class, so we can instruct Spring where to look for classes annotated with ```@Component```*
...
```
*Instance of the class Parrot is added to the Spring Context because it is annotated with ```@Component```*

By default **Spring doesn't serach for classes annotated with stereotype annotations**, so we have to tell him where to search using the ```@ComponentScan``` annotation over the configuration class 

```java
@Configuration
// basePackage is used to enumerate the packages where we defined the classes with @Component annotation
@ComponentScan(basePackage = "main")
public class ProjectConfig {
}
```

*Note: Getting a bean from the context is the same using the context.getBean(...) function, but once returned the bean wont have any values assigned, so you have to assign values to it*

Since annotation with ```@Component``` returns us empty instance from the Spring context, we can create a method that is called when the instance is about to be put in the Spring context, using ```@PostConstruct``` annotation

*First in order to do that you need to add **Jakarta Annotations API** depedency, since it is not part of the JDK*
...
public class Parrot {
  ...
  // this method will be called when the instance is about to be put in the spring context
  @PostConstruct
  public void init() {
    this.name = "Kiki";
  }
